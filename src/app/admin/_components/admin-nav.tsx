...
export function AdminNav({ active }: { active: string }) {
  return (
    <nav className="flex items-center gap-1 border-b border-surface-border pb-3 text-sm">
      {LINKS.map((link) => (
        <Link
          key={link.href}
          href={link.href}
          className={
            link.href === active
              ...
              : "rounded-md px-3 py-1.5 text-text-secondary transition-colors duration-150 hover:text-text-primary"
          }
        >
          {link.label}
        </Link>
